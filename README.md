...
- ✅ 通用网关支持：
  - `https://网关域名/https://源站host:port`（**强制 HTTPS 回源**）
...
* 使用 `http://` 前缀（**推荐用于“端口实际是 HTTP”的情况**）：
...
> ⭐ **如何判断该用 `http://` 还是 `https://`？**
> 在 VPS 上测试：
>
> * `curl -vk https://目标:端口/` 如果出现 `wrong version number` / TLS 握手失败 → **该端口不是 HTTPS**，用 `http://` 前缀。
> * `curl -v http://目标:端口/` 能正常 200/302 → 也说明是 **HTTP**，用 `http://` 前缀。

> ✅ 兼容建议
>
> * 新格式要求路径里直接写完整上游 URL，例如 `https://网关域名/https://源站:端口`。
